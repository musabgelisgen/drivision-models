...
sign_recognition_label_names = open("signnames.csv").read().strip().split("\n")[1:]
...
sign_recognition_model = TrafficSignNet.build(width=32, height=32, depth=3,
                                              classes=numLabels)
sign_recognition_model.compile(loss="categorical_crossentropy", optimizer=opt,
                               metrics=["accuracy"])

# compile the model and train the network
print("[INFO] training network...")
H = sign_recognition_model.fit_generator(
...
predictions = sign_recognition_model.predict(testX, batch_size=BS)
print(classification_report(testY.argmax(axis=1),
							predictions.argmax(axis=1), target_names=sign_recognition_label_names))

# save the network to disk
print("[INFO] serializing network to '{}'...".format(args["model"]))
sign_recognition_model.save(args["model"])
