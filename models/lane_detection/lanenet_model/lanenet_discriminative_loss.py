...
def discriminative_loss_single(
    ...
    distance = tf.norm(tensor=tf.subtract(mu_expand, reshaped_pred), axis=1)
    ...
    mu_norm = tf.norm(tensor=mu_diff_bool, axis=1)
    ...
    l_reg = tf.reduce_mean(input_tensor=tf.norm(tensor=mu, axis=1))
